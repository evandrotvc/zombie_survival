...
class ReportsController < ApplicationController
  def infected_percentage
    ...
    percentage = (infected_users.to_f / total_users) * 100
  ...
  def non_infected_percentage
    ...
    total_users = User.count
    percentage = (non_infected_users.to_f / total_users) * 100
    render plain: "Percentage of non-infected users: #{percentage}%"
  end

  def average_items_per_user
    calculate_average_water_food
    calculate_average_ammunition_remedy

    render plain: 'Average quantity of each type of item per user: water/user:' \
                  "#{@average_waters}, food/user: #{@average_foods}" \
                  "ammunition/user: #{@average_ammunitions}," \
                  "Remedies/user: #{@average_remedies}"
  end
